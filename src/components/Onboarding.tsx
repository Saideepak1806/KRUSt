...
import { Compass, Sparkles, BookOpen, Layers, Check, Plus, Trash2, Brain, Loader } from 'lucide-react';
    ...
    try {
      ...
      if (onGenerateAICareer) {
        ...
        const cleanCareer: Career = {
          ...
          skillIds: rawSkillIds.map((sid: string) => skillIdMap[sid] || sid),
          weights: Object.entries(rawCareer.weights || {}).reduce((acc: Record<string, number>, [sid, w]) => {
            acc[skillIdMap[sid] || sid] = Number(w);
            return acc;
          }, {}),
  ...
  return (
    <div className="max-w-7xl w-full mx-auto py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* Right 1 column: Custom Career Creator or Side Panel */}
        <div className="lg:col-span-1">
          {showCreator ? (
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              className="bg-slate-950 p-6 rounded-xl border border-slate-800 sticky top-6 shadow-xl"
            >
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-800/80">
                ...
                <span className="text-[10px] font-mono text-emerald-400 font-bold">DOMAIN INDEPENDENT</span>
              </div>
                    ...
                    <>
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. Sales Specialist or Head Chef"
                          className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3.5 py-2 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-emerald-500 transition-colors font-mono"
                        />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="Focus on pipeline control, CRM software, leadership, or custom culinary preparation standards..."
                          rows={3}
